...
deploy-latest-image-init-webhook-local:
    $(MAKE) build-init-webhook -e BUILD_ENVIRONMENT=main 
...
deploy-latest-image-webhook-server-local:
    $(MAKE) build-webhook-server -e BUILD_ENVIRONMENT=main
    docker tag webhook-server ${LOCAL_REGISTRY}/webhook-server:${IMAGE_TAG}
    docker push ${LOCAL_REGISTRY}/webhook-server:${IMAGE_TAG}

TOKEN := 62a8574c-643f-4127-a897-7f18a9023e6d
install-latest-datree-awsmp-local:
    $(MAKE) deploy-image-init-webhook-local
    $(MAKE) deploy-image-cert-generator-local
    $(MAKE) deploy-image-webhook-server-local
